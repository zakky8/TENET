...
4. **CVE-class mistakes prevented by construction.** No `pickle`-style deserialization. Parameterized queries. Regex-validated filter keys. Path-allowlist on every file I/O via `PathHandle` + `openPath()`. `Secret<T>` opaque type refuses `JSON.stringify`. Trivy + `pnpm audit` in CI (report-only pre-1.0); secrets blocked pre-commit (gitleaks).
