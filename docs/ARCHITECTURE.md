...
- **Repo:** 63 workspace packages (every `package.json` under the `pnpm-workspace.yaml`
  globs). Test suite: **98 suites / 1320 tests, all green** (was 83 / 1051 at the start
...
- `eval/harness` running TENET's existing golden conversations against the new runtime (the repo-wide unit/suite figure — 98 suites / 1320 tests green as of 2026-07-18 — is the hermetic stub plane, not a real-model eval)
