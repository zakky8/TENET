...
4. **Fail CLOSED on infra failure.** A verifier throw, a judge timeout, a retriever error, a truncated (`max_tokens`) model response, an over-cap claim extraction — all should resolve to abstain, never ship. Truncation especially: a response cut off at the token limit may be incomplete, so treat `stopReason: max_tokens` as abstain. *Reference: the **opt-in** `failClosed` mode in `@tenet/verifier` (default is fail-OPEN — enable it), plus the reasoner's stop-reason handling and the orchestrator's top-level catch.*
